...
- (NSString *)description{
	return [NSString stringWithFormat:@"<%@ = 0x%08x> %@", [self respondsToSelector:@selector(className)] ? [self className] : @"null", (long)self, source];
}
...
timeToLive: %f, timestamp: %f,\nheaders:\n%@,\nbody:\n%@", [self respondsToSelector:@selector(className)] ? [self className] : @"null", (long)self, clientId, 
...
- (NSString *)description{
	return [NSString stringWithFormat:@"<%@ = 0x%08x> faultCode: %@, faultDetail: %@, faultString: %@", 
		[self respondsToSelector:@selector(className)] ? [self className] : @"null", (long)self, faultCode, faultDetail, faultString];
}
