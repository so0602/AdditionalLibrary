...
- (void)_ensureLength:(unsigned)length{
	if (m_position + length > [m_data length]){
		[NSException raise:@"NSUnarchiverBadArchiveException"
			...
			[self respondsToSelector:@selector(className)] ? [self className] : @"null", m_position, length, [m_data length]];
	}
}

- (void)_cannotDecodeType:(const char *)type{
	[NSException raise:@"NSUnarchiverCannotDecodeException"
		format:@"%@ cannot decode type=%s", [self respondsToSelector:@selector(className)] ? [self className] : @"null", type];
}

- (id)_objectReferenceAtIndex:(uint32_t)index{
	if ([m_objectTable count] <= index){
		[NSException raise:@"NSUnarchiverCannotDecodeException" 
			format:@"%@ cannot decode object reference", [self respondsToSelector:@selector(className)] ? [self className] : @"null"];
	}
...
- (NSString *)_stringReferenceAtIndex:(uint32_t)index{
	if ([m_stringTable count] <= index){
		[NSException raise:@"NSUnarchiverCannotDecodeException" 
			format:@"%@ cannot decode string reference", [self respondsToSelector:@selector(className)] ? [self className] : @"null"];
	}
...
- (AMF3TraitsInfo *)_traitsReferenceAtIndex:(uint32_t)index{
	if ([m_traitsTable count] <= index){
		[NSException raise:@"NSUnarchiverCannotDecodeException" 
			format:@"%@ cannot decode traits reference", [self respondsToSelector:@selector(className)] ? [self className] : @"null"];
	}
